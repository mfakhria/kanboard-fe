...
function getQaColor(i: number): { bg: string; accent: string } {
  return i < qaColors.length ? qaColors[i]! : qaColors[0]!
}
...
const notifIconMap: Record<string, { bg: string; color: string; icon: Component }> = {
...
const defaultNotifIcon = notifIconMap['info']!

function getNotifStyle(type: string): { bg: string; color: string; icon: Component } {
  return notifIconMap[type] ?? defaultNotifIcon
}
...
<template>
  ...
  <header
    class="sticky top-0 z-30 flex items-center shrink-0 bg-white dark:bg-gray-900"
    style="height: 64px; border-bottom: 1px solid var(--hdr-border, #edf0f4); box-shadow: 0 1px 0 var(--hdr-border, #edf0f4), 0 2px 8px rgba(0,0,0,0.03); padding: 0 20px;"
  >
    ...
    <div class="flex items-center gap-2 shrink-0" style="min-width: 180px;">
      ...
      <div class="hidden lg:flex items-center gap-2">
        <div
          ...
          style="width: 32px; height: 32px; background: linear-gradient(135deg, #edf4ff, #dbeafe); color: #478FC8;"
        >
    ...
    <div class="flex-1 flex justify-center px-4 lg:px-6">
      <div ref="searchWrapperRef" class="relative w-full" style="max-width: 420px;">
        ...
        >
          ...
          <div v-else class="flex items-center gap-0.5 shrink-0 rounded-lg px-1.5 py-0.5" style="background: #eef0f3; border: 1px solid #e2e6ea;">
        ...
        <div
          ...
          style="width: min(440px, 95vw); background: #fff; border-radius: 16px; border: 1px solid #e8edf3; box-shadow: 0 20px 60px rgba(0,0,0,0.12), 0 4px 16px rgba(0,0,0,0.06); max-height: 440px; overflow-y: auto;"
        >
          <div v-if="searchResults.length === 0" class="flex flex-col items-center justify-center gap-2 py-10">
            ...
            <p class="text-[13.5px] font-semibold text-gray-500">No results found</p>
            <p class="text-[12px] text-gray-400">Try searching with different keywords</p>
          </div>
          <template v-else>
            <div v-for="(group, gi) in groupedResults" :key="group.type">
              <div class="flex items-center gap-1.5 px-4 pt-3.5 pb-1.5" :style="{ borderTop: gi > 0 ? '1px solid #f1f5f9' : 'none' }">
              ...
              <div
                ...
                :style="{ background: searchResults.indexOf(item) === activeIdx ? '#edf4ff' : 'transparent' }"
              ...
              >
                <div
                  ...
                  :style="{ width: '34px', height: '34px', background: searchResults.indexOf(item) === activeIdx ? '#dbeafe' : '#f1f5f9', color: searchResults.indexOf(item) === activeIdx ? '#478FC8' : '#64748b' }"
                >
                  <component :is="item.icon ?? FolderKanban" style="width: 14px; height: 14px;" />
                </div>
                <div class="flex-1 min-w-0">
                  <p class="truncate text-[13px] font-semibold" :style="{ color: searchResults.indexOf(item) === activeIdx ? '#2d6da3' : '#1e293b' }">
                    <template v-if="typeof highlightText(item.title, searchQuery) === 'string'">{{ item.title }}</template>
                    <template v-else>
                      {{ (highlightText(item.title, searchQuery) as { before: string; match: string; after: string }).before }}<mark style="background: #dbeafe; color: #1d4ed8; border-radius: 2px; padding: 0 1px;">{{ (highlightText(item.title, searchQuery) as { before: string; match: string; after: string }).match }}</mark>{{ (highlightText(item.title, searchQuery) as { before: string; match: string; after: string }).after }}
                    </template>
                  </p>
            ...
            <div class="flex items-center justify-between px-4 py-2.5" style="border-top: 1px solid #f1f5f9; background: #fafbfc;">
              <div class="flex items-center gap-3">
                <div v-for="k in [{ key: '↑↓', label: 'navigate' }, { key: '↵', label: 'select' }, { key: 'Esc', label: 'close' }]" :key="k.key" class="flex items-center gap-1">
                  <kbd class="flex items-center justify-center rounded text-[10px] font-semibold" style="min-width: 20px; height: 18px; padding: 0 4px; background: #e8edf3; color: #64748b;">{{ k.key }}</kbd>
                  <span class="text-[10.5px] text-gray-400">{{ k.label }}</span>
                </div>
              </div>
        ...
        <div
          v-if="searchOpen && searchQuery.trim().length === 0"
          class="absolute left-1/2 -translate-x-1/2 top-full mt-2 z-50 flex flex-col overflow-hidden"
          style="width: min(440px, 95vw); background: #fff; border-radius: 16px; border: 1px solid #e8edf3; box-shadow: 0 20px 60px rgba(0,0,0,0.12), 0 4px 16px rgba(0,0,0,0.06);"
        >
          ...
          >
            ...
            <div class="flex-1 min-w-0">
              <p class="text-[13px] font-semibold text-gray-900">{{ item.title }}</p>
              <p class="text-[11.5px] text-gray-400">{{ item.subtitle }}</p>
            </div>
            <Clock style="width: 12px; height: 12px; color: #e2e8f0;" />
          </div>
          <div class="flex items-center gap-2 px-4 py-2.5" style="border-top: 1px solid #f1f5f9; background: #fafbfc;">
    ...
    <div class="flex items-center gap-2 shrink-0" style="min-width: 180px; justify-content: flex-end;">
      <!-- Icon pill group -->
      <div class="hidden sm:flex items-center gap-0.5 rounded-2xl px-1 py-1" style="background: #f6f8fa; border: 1.5px solid #edf0f4;">
        <button
          class="relative w-8 h-8 flex items-center justify-center rounded-xl transition-all duration-150 text-gray-500 hover:bg-[#edf0f4]"
        ...
        <button class="relative w-8 h-8 flex items-center justify-center rounded-xl transition-all duration-150 text-gray-500 hover:bg-[#edf0f4]" title="Messages">
        ...
        <div class="relative">
          <button
            ...
            :class="notifOpen ? 'text-[#478FC8] bg-[#dbeafe]' : 'text-gray-500 hover:bg-[#edf0f4]'"
            title="Notifications"
            @click="notifOpen = !notifOpen; profileOpen = false"
          >
            <Bell style="width: 15px; height: 15px;" />
            <span v-if="unreadCount > 0" class="absolute rounded-full" style="top: 6px; right: 6px; width: 8px; height: 8px; background: linear-gradient(135deg, #f97316, #ef4444); border: 1.5px solid #f6f8fa;" />
          </button>
          ...
          <div
            ...
            style="width: 360px; background: #fff; border-radius: 18px; border: 1px solid #e8edf3; box-shadow: 0 20px 60px rgba(0,0,0,0.12), 0 4px 16px rgba(0,0,0,0.06);"
          >
            <div class="flex items-center justify-between px-5 py-3.5" style="border-bottom: 1px solid #f1f5f9;">
              <div class="flex items-center gap-2.5">
                <div class="w-7 h-7 rounded-xl flex items-center justify-center" style="background: linear-gradient(135deg, #edf4ff, #dbeafe);">
                  <Bell style="width: 13px; height: 13px; color: #478FC8;" />
                </div>
                <span class="text-[13.5px] font-bold text-gray-900">Notifications</span>
              ...
              <div class="flex items-center gap-1">
                ...
                <button class="w-7 h-7 flex items-center justify-center rounded-lg text-gray-400 hover:bg-gray-100 transition-colors" @click="notifOpen = false">
            ...
            <div style="max-height: 300px; overflow-y: auto;">
              <div
                ...
                :key="n.id"
                class="flex items-start gap-3 px-5 py-3.5 cursor-pointer hover:bg-gray-50/80 transition-colors"
                :style="{ opacity: n.read ? 0.5 : 1, borderBottom: i < notifList.length - 1 ? '1px solid #f8fafc' : 'none' }"
              >
                ...
                <div class="flex-1 min-w-0">
                  <div class="flex items-baseline justify-between gap-2">
                    <p class="truncate text-[12.5px] text-gray-900" :style="{ fontWeight: n.read ? 500 : 650 }">{{ n.title }}</p>
            ...
            <div class="px-5 py-3" style="border-top: 1px solid #f1f5f9; background: #fafbfc;">
              <button class="w-full py-2 rounded-xl text-[#478FC8] hover:bg-[#edf4ff] transition-colors text-[12.5px] font-semibold">
      ...
      <!-- Divider -->
      <div class="hidden md:block" style="width: 1px; height: 24px; background: #edf0f4;" />
      ...
      <div class="relative">
        ...
        >
          <div class="relative shrink-0">
            ...
            <div class="absolute rounded-full" style="width: 9px; height: 9px; bottom: -1px; right: -1px; background: #22c55e; border: 2px solid #fff;" />
          </div>
        ...
        <div
          ...
          style="width: 220px; background: #fff; border-radius: 18px; border: 1px solid #e8edf3; box-shadow: 0 20px 60px rgba(0,0,0,0.12), 0 4px 16px rgba(0,0,0,0.06);"
        >
          ...
          <div style="position: relative; margin-top: -20px; padding-left: 16px;">
            <div
              ...
              style="width: 40px; height: 40px; font-size: 13px; font-weight: 700; background: linear-gradient(135deg, #478FC8, #5BA3D9); box-shadow: 0 0 0 3px #fff, 0 4px 12px rgba(71,143,200,0.4);"
            >
              {{ userInitials }}
            </div>
          </div>
          <div class="px-4 pb-3 pt-1">
            <p class="text-[13px] font-bold text-gray-900">{{ authStore.currentUser?.name || 'User' }}</p>
            <p class="text-[11px] text-gray-400 mt-[1px]">{{ authStore.currentUser?.email || '' }}</p>
            <span
          ...
          <div class="mx-3 h-px" style="background: #f1f5f9;" />
          <div class="p-2">
            <button class="w-full flex items-center gap-2.5 px-3 py-2.5 rounded-xl text-gray-600 hover:bg-gray-50 transition-colors text-[13px] font-medium" @click="closeAll(); router.push('/dashboard/settings')">
              <div class="w-6 h-6 rounded-lg flex items-center justify-center" style="background: #f1f5f9;"><User style="width: 12px; height: 12px; color: #64748b;" /></div>
              My Profile
            </button>
            <button class="w-full flex items-center gap-2.5 px-3 py-2.5 rounded-xl text-gray-600 hover:bg-gray-50 transition-colors text-[13px] font-medium" @click="closeAll(); router.push('/dashboard/settings')">
              <div class="w-6 h-6 rounded-lg flex items-center justify-center" style="background: #f1f5f9;"><Settings style="width: 12px; height: 12px; color: #64748b;" /></div>
              Settings
            </button>
            <div class="my-1 mx-1 h-px bg-gray-100" />
            <button class="w-full flex items-center gap-2.5 px-3 py-2.5 rounded-xl text-red-500 hover:bg-red-50 transition-colors text-[13px] font-medium" @click="handleLogout">
              <div class="w-6 h-6 rounded-lg flex items-center justify-center" style="background: #fff1f2;"><LogOut style="width: 12px; height: 12px; color: #ef4444;" /></div>
              Sign out
            </button>
          </div>
        </div>
