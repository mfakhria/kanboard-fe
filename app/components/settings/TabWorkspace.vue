...
<template>
  <div class="flex flex-col gap-5">
    <!-- General -->
    <SettingsSection :title="t('settings.workspace.generalTitle')" :description="t('settings.workspace.generalDesc')">
      <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
        <SettingsFormField
          v-model="workspaceName"
          :label="t('settings.workspace.teamName')"
          :icon="Building2"
        />

        <!-- Timezone -->
        <div class="flex flex-col gap-1.5">
          <label class="text-gray-600 dark:text-gray-400 text-[12.5px] font-semibold">{{ t('settings.workspace.timezone') }}</label>
          <div class="relative">
        ...
        <!-- Language -->
        <div class="flex flex-col gap-1.5">
          <label class="text-gray-600 dark:text-gray-400 text-[12.5px] font-semibold">{{ t('settings.workspace.language') }}</label>
          <div class="relative">
            <div class="absolute left-3 top-1/2 -translate-y-1/2 text-gray-400 dark:text-gray-500">
            ...
            <select
              ...
              class="w-full pl-9 pr-4 py-2.5 rounded-xl border border-gray-200 dark:border-gray-700 bg-gray-50/50 dark:bg-gray-800/50 text-gray-800 dark:text-gray-200 outline-none transition-all focus:border-[#478FC8] focus:bg-white dark:focus:bg-gray-800 focus:ring-2 focus:ring-[#478FC8]/10 appearance-none text-[13.5px]"
            >
              <option value="en">{{ t('settings.workspace.languageEnglish') }}</option>
              <option value="id">{{ t('settings.workspace.languageIndonesian') }}</option>
            </select>
          </div>
        </div>
        ...
        <!-- Date Format -->
        <div class="flex flex-col gap-1.5">
          <label class="text-gray-600 dark:text-gray-400 text-[12.5px] font-semibold">{{ t('settings.workspace.dateFormat') }}</label>
          <div class="relative">
            <div class="absolute left-3 top-1/2 -translate-y-1/2 text-gray-400 dark:text-gray-500">
              <Clock style="width: 14px; height: 14px;" />
            </div>
    ...
    <!-- Preferences -->
    <SettingsSection :title="t('settings.workspace.preferencesTitle')" :description="t('settings.workspace.preferencesDesc')">
      <div class="flex flex-col divide-y divide-gray-50 dark:divide-gray-800">
        <div class="flex items-center justify-between py-3 first:pt-0">
          <div>
            <p class="text-[13.5px] font-semibold text-gray-900 dark:text-white">{{ t('settings.workspace.autoArchive') }}</p>
            <p class="text-gray-400 dark:text-gray-500 text-[12px]">{{ t('settings.workspace.autoArchiveDesc') }}</p>
          </div>
        ...
        <div class="flex items-center justify-between py-3 last:pb-0">
          <div>
            <p class="text-[13.5px] font-semibold text-gray-900 dark:text-white">{{ t('settings.workspace.publicTasks') }}</p>
            <p class="text-gray-400 dark:text-gray-500 text-[12px]">{{ t('settings.workspace.publicTasksDesc') }}</p>
          </div>
    ...
    <!-- Data Management -->
    <SettingsSection :title="t('settings.workspace.dataTitle')" :description="t('settings.workspace.dataDesc')">
      <div class="flex flex-col gap-3 sm:flex-row">
        ...
        <button class="flex items-center gap-2 px-5 py-2.5 rounded-xl border border-red-200 dark:border-red-800 text-red-500 dark:text-red-400 hover:bg-red-50 dark:hover:bg-red-900/20 transition-all text-[13px] font-semibold">
          ...
          {{ t('settings.workspace.deleteTeam') }}
        </button>
      </div>
    </SettingsSection>
  </div>
